...
def main() -> None:
    ...
    print(f"Recording {seconds:.1f}s of mono 16-bit audio at {SAMPLE_RATE} Hz. Speak now...")
    try:
    ...
    if recording.size == 0 or not np.any(recording):
        print("FAIL: recording buffer is empty or entirely silent (dtype/permission issue?).")
        raise SystemExit(1)
